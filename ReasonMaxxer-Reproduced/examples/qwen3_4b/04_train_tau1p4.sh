...
$PYTHON scripts/prepare_training_data.py \
  ...
  --seed 42 \
  --target_ids_output "$ROOT/train/target_ids_tau1p4.json" \
  --processed_output "$ROOT/train/processed_tau1p4.json" \
  --training_examples_output "$ROOT/train/training_examples_tau1p4.json" \
  --stats_output "$ROOT/train/training_stats_tau1p4.json" \
  --selected_records_output "$ROOT/train/selected_records_tau1p4.json"

CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} $PYTHON scripts/train_reasonmaxxer.py \
  --training_data "$ROOT/train/processed_tau1p4.json" \
